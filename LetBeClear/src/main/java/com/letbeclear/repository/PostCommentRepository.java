...
public interface PostCommentRepository extends JpaRepository<PostComment, Integer>
{
	...
	@Transactional
	public int insertCommentPost(@Param("postId")Long postId, @Param("comment")String comment, @Param("userId")Long userId);
...
//	@Transactional
	...
	public List<PostComment> getAllComments(@Param("postId")Long postId);
...
//	public List<PostComment> getAllComments(Integer postId);
	...
	@Transactional
	public int editOldComment(long commentId, String comment);

	@Modifying
	@Transactional
	@Query(value="delete from post_comment where comment_id=:commentId", nativeQuery=true)
	public int deleteComment(@Param("commentId") Long commentId);
	
	public int countByUserPost(UserPost userPost);
	
	@Query("select count(p) from PostComment p where p.userPost.postId=?1")
	public int countByPostId(long postId);

	@Query("select p.users.userId from PostComment p where p.commentId=?1")
